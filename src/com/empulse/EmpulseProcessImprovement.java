...
public class EmpulseProcessImprovement {
	...
	public static void main(String[] args) {
		...
		try {
			String url = "https://www.empulseudp.com/ProcessImprovement/";
			driver.get(url);
			String beforelogin = driver.getPageSource();
		    DateFormat dateFormat = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
			Date datestart = new Date();
			FileWriter fw = new FileWriter("test4(ProcessImp)_log.txt", true);
			...
			if (driver.getTitle().contains("404") || beforelogin.contains("Access Denied"))
			{
				...
				if (driver.getTitle().contains("404")) {
					fw.write(dateFormat.format(datestart) + "\t" + "Failure - (404 received before login)\t"+url+"\n");
				...
				driver.findElement(By.xpath("//input[@formcontrolname='username']")).sendKeys("sherlin.thomas@empulseglobal.com");
				...
				if (driver.getTitle().contains("404") || beforelogin.contains("Access Denied"))
				{
					System.out.println("\nWe are in first if block");
					if (driver.getTitle().contains("404")) {
						fw.write(dateFormat.format(datestart) + "\t" +"Failure - (404 received after login)\t"+url+"\n");
					...
					fw.write(dateFormat.format(datestart)+ "\t" + "Success - (no error encountered)\t" + url+"\n");
					Date datestop_after = new Date();
					long diff_before = datestop_before.getTime() - datestart.getTime();
					long diffSeconds_before = diff_before / 1000;         
					long diffMinutes_before = diff_before / (60 * 1000);
